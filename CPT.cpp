...
void CPT_Animation(void(*s_World)(double, World*), point(*s_Camera)(double), point(*s_ViewPoint)(double), double(*s_SolidAngle)(double),
	double t0, double t1, unsigned canvas_width, unsigned canvas_height, unsigned fps, unsigned render_sampling, string filename_header, unsigned filename_reserve) {
...
void CPT_Animation_R() {
	...
	//const unsigned width = 768, height = 432, sampling = 1, fps = 25;
	const unsigned R = 502;
...
#pragma region R0
	if (R == -1 || R == 0) CPT_Animation([](double t, World* W) {	// World
		...
		*((XSolid*)(X.Objs[1])) = CSG_Translation(CSG_Scale(*((XSolid*)(X.Objs[1])), 1.8), point(9.5, 1.5, 0.72));
		...
		return 0.15;
	}, 0, 12, width, height, fps, sampling, "Animation\\CPTAR0_", 3);	// beginning
...
#pragma region R2
	...
	if (R == -1 || R == 2 || R == 202) CPT_Animation([](double t, World* W) {	// World
		...
		return 0.15;
	}, 16, 20, width, height, fps, sampling, "Animation\\CPTAR2.2_", 3);	// 16s-20s, golden coin shining
...
#pragma region R3-4
	if (R == -1 || R == 3 || R == 4) CPT_Animation([](double t, World* W) {	// World
		parallelogram_grid P(parallelogram(point(-3, -ERR_UPSILON, 0), point(7, 0), point(0, 2 * ERR_UPSILON))); W->add(P);
		GlassMan_std G0; SetWalkingAttitudes(t, G0); G0.Pos += point(-0.4, 0); G0.push(*W);
		GlassMan_std G1; SetWalkingAttitudes(t + 0.4, G1); G1.Pos += point(3.2, 12); G1.push(*W);
		GlassMan_std G3; SetWalkingAttitudes(t - 0.7, G3); G3.Pos += point(-1.2, 32); G3.push(*W);
		...
		point Pc(4 * sigmoid(10 * (t - 22)), 0, 0);
		parallelogram_grid Ps(parallelogram(Pc + point(5, 6, -0.5), point(2.4, 0), point(0, 2.4)), 0.8, 0.8);
		W->insert(&GoldenCoin_Constructor(Pc + point(6.2, 7.2, -0.1 - (t < 22.1 ? 0 : 100 * (t - 22.1)*(t - 22.1))), 0.2));

		//ManStaringAtTheGoldenCoin_Constructor(point(1.8, 2.3), point(6.2, 7.2, -0.1)).push(*W);		// Protagonist
		GlassMan_std G(RunningAttitude_Constructor(t - 21, vec3(point(1.8, 2.3), point(6.2, 7.2))));
		G.Pos += point(1.8, 2.3); swap(G.dir.x, G.dir.y), G.dir.y = -G.dir.y;	// t = 21.6 => (4.01409,4.76569,0)
		G.Pos = G.Pos + (1.0, G.dir.mod() / G.dir.x, sigmoid(30 * (t - 21.8))) * (G.Pos - point(4, 4.8));
		G.Pos.y = mix(G.Pos.y, 7.2, sigmoid(30 * (t - 21.8))), G.Pos.z = mix(G.Pos.z, -4.9*(t - 21.8)*(t - 21.8), sigmoid(30 * (t - 21.8)));
		G.dir = mix(G.dir, vec3(0.2, -1, 0), sigmoid(40 * (t - 21.8)));

		GlassMan_std Gu(point(0.27994, 1.1312, 1.4000), point(0.28128, 0.92005, 1.2527), point(0.14230, 0.83297, 1.1320), point(0.42432, 0.82556, 1.1223), point(0.12015, 1.1334, 1.1566), point(0.47705, 1.1397, 1.1553), point(0.097157, 1.4300, 1.1398), point(0.49427, 1.4179, 1.0861), point(0.28516, 0.63289, 0.93746),
			point(0.20566, 0.42691, 0.95590), point(0.34199, 0.43178, 0.93671), point(0.18489, 0.13723, 1.0383), point(0.35332, 0.13946, 1.0000), point(0.15868, -0.30879, 1.0310), point(0.37014, -0.30166, 1.0000), point(0.14681, -0.44522, 1.0000), point(0.36885, -0.42619, 1.0000));
		Gu.dir = G.dir; Gu.Pos = point(7.2, 7.2); Gu.Pos = G.Pos;
		G = mix(G, Gu, sigmoid(10 * (t - 21.8)));
		G.auto_fit = true;

		if (G.Pos.z < Pc.z - 0.5) {
			double df = Pc.z - 0.5 - G.Pos.z;
			G.Pos.z += df;
			G = mix(G, ([](const GlassMan_std &G)->GlassMan_std {
				GlassMan_std Gu(point(0.27994, 1.1312), point(0.28128, 0.92005), point(0.14230, 0.83297), point(0.42432, 0.82556), point(0.12015, 1.1334), point(0.47705, 1.1397), point(0.097157, 1.4300), point(0.49427, 1.4179), point(0.28516, 0.63289), point(0.20566, 0.42691), point(0.34199, 0.43178), point(0.18489, 0.13723), point(0.35332, 0.13946), point(0.15868, -0.30879), point(0.37014, -0.30166), point(0.14681, -0.44522), point(0.36885, -0.42619));
				Gu.Pos = G.Pos; Gu.dir = G.dir; Gu.auto_fit = true; return Gu;
			})(G), 1 - exp(-2 * df));
			Pc.x += tanh(df), Ps.O.x += tanh(df);
			G.Pos = mix(G.Pos, Pc + point(6.2, 7.2, -0.5), clamp(df, 0.0, 1.0));
		}
		...
		return Catmull_Rom(vector<point>({ point(0, 3, 2.8), point(0, -6, 2), point(7.2, -1, 1.4), point(8.6, 1, 1) }),
			sigmoid(20 * (t - 21.0)) + sigmoid(15 * (t - 21.6)) + sigmoid(15 * (t - 22)));
	}, [](double t) -> point {	// view point
		//return point(10, 7.2, 0);
		return Catmull_Rom(vector<point>({ point(6.2, 7.2, -0.4), point(9, 7, 0), point(10, 7.2, 0) }), sigmoid(10 * (t - 22.0)) + sigmoid(10 * (t - 22.2)));
	}, [](double t) -> double {		// solid angle
		//return 0.15;
		return mix(mix(0.15, 0.3, sigmoid(20 * (t - 20.6))), 0.5, sigmoid(10 * (t - 22)));
	}, 20, 24, width, height, fps, sampling, "Animation\\CPTAR3_", 3);	// 20s-24s

#pragma endregion The glass man saw it and stopped, then jumped onto the platform to get the golden coin.  \
  R4. As he lands on the platform, the coin disappeared.

#pragma region R5
	if (R == -1 || R == 5 || R == 501) CPT_Animation([](double t, World* W) {	// World
#pragma region
		parallelogram_grid P(parallelogram(point(-3, -ERR_UPSILON, 0), point(7, 0), point(0, 2 * ERR_UPSILON))); W->add(P);
		GlassMan_std G0; SetWalkingAttitudes(t, G0); G0.Pos += point(-0.4, 0); G0.push(*W);
		GlassMan_std G1; SetWalkingAttitudes(t + 0.4, G1); G1.Pos += point(3.2, 12); G1.push(*W);
		...
		parallelogram_grid Ps(parallelogram(point(9, 6, -0.5), point(2.4, 0), point(0, 2.4)), 0.8, 0.8);
		...
		if (G.Pos.z < -0.5) {
			...
			Ps.O.x++;
			G.Pos = mix(G.Pos, point(11.2, 7.2, -0.5), clamp(df, 0.0, 1.0));
		}

		W->add(Ps);
		G.push(*W);
#pragma endregion exactly the same as R4
	}, [](double t) -> point {	// camera
		//return Catmull_Rom(vector<point>({ point(8.6, 1, 1), point(14, -16, 5), point(-11, -4, 16) }), 2 * sigmoid(12 * (t - 24.5)));
		return Catmull_Rom(vector<point>({ point(8.6, 1, 1), point(14, -16, 5), point(-11, -4, 16) }), sigmoid(15 * (t - 24.4)) + sigmoid(15 * (t - 25.6)));
	}, [](double t) -> point {	// view point
		return Catmull_Rom(vector<point>({ point(10, 7.2, 0), point(2, 3, 1) }), sigmoid(6 * (t - 25)));
	}, [](double t) -> double {		// solid angle
		return 0.5;
	}, 24, 28, width, height, fps, sampling, "Animation\\CPTAR5.1_", 3);	// 24s-28s

	if (R == -1 || R == 5 || R == 502) CPT_Animation([](double t, World* W) {	// World
		parallelogram_grid P(parallelogram(point(-3, -ERR_UPSILON, 0), point(7, 0), point(0, 2 * ERR_UPSILON))); W->add(P);
		GlassMan_std G0; SetWalkingAttitudes(t, G0); G0.Pos += point(-0.4, 0); G0.push(*W);
		GlassMan_std G1; SetWalkingAttitudes(t + 0.4, G1); G1.Pos += point(3.2, 12); G1.push(*W);
		GlassMan_std G3; SetWalkingAttitudes(t - 0.7, G3); G3.Pos += point(-1.2, 32); G3.push(*W);
		...
		point Pos(10.2, 7.2, -0.5 - 1.0 * (t - 30));
		parallelogram_grid Ps(parallelogram(Pos - vec3(1.2, 1.2), point(2.4, 0), point(0, 2.4)), 0.8, 0.8); W->add(&Ps);

		GlassMan::saperated_waist = true;
		GlassMan_std G(point(0.29165, 0.63077, 1.72862), point(0.28804, 0.64743, 1.5), point(0.12473, 0.67233, 1.32706), point(0.44241, 0.57127, 1.33198), point(0.10081, 0.69408, 1.0525), point(0.47014, 0.60476, 1.03801), point(0.06629, 0.74147, 0.63333), point(0.53982, 0.68705, 0.63476), point(0.28804, 0.63169, 1.02906), point(0.19516, 0.6725, 0.93665), point(0.3801, 0.63518, 0.93671), point(0.16257, 0.67054, 0.53541), point(0.39826, 0.64483, 0.55453), point(0.11795, 0.62541, 0.06794), point(0.4, 0.6, 0.07204), point(0.1169, 0.71535, 0), point(0.4, 0.7, 0)); G.Pos = Pos, G.dir = vec3(0, 1, 0), G.auto_fit = true;
		G = mix(([](point Pos)->GlassMan_std { GlassMan_std G = ManStaringAtTheGoldenCoin_Constructor(Pos, Pos + vec3(0, 1, 0)); G.dir = vec3(0, 1, 0); return G; })(Pos), G, 0.4);
		XSolid X; G.construct(X); X.type = XSolid_Crystal; W->add(&X);
		GlassMan::saperated_waist = false;

	}, [](double t) -> point {	// camera
		point P = Catmull_Rom(vector<point>({ point(6, 0, 1.8), point(12.6, -1.7, 1.6), point(20, 5.2, 1.5) }), 2 * tanh(4 * (t - 30)));
		P.z -= 1 - 1.0 * (t - 30);
		return P;
	}, [](double t) -> point {	// view point
		point P(11.2, 7.2, 0.5);
		P.z -= 1.0 * (t - 30);
		return P;
	}, [](double t) -> double {		// solid angle
		return 0.2;
	}, 30, 32, width, height, fps, sampling, "Animation\\CPTAR5.2_", 3);	// 30s-32s, 2s delayed

#pragma endregion The glass man looks around. His peers are still walking on the path.
}
