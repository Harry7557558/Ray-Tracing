...
public:
	...
	XSolid construct() {
		...
		double rz = atan2(dir.y, dir.x), ry = atan2(-dir.z, hypot(dir.x, dir.y)), rx = acos(top.z / cos(ry));
		Body = XSolid();
		...
		XSolid tmp_FEET = CSG_UnionOp(tmp_FOOT_L, tmp_FOOT_R);
		...
		XSolid tmp_SHANKS = CSG_UnionOp(tmp_SHANK_L, tmp_SHANK_R);
		...
		XSolid tmp_THIGHS = CSG_UnionOp(tmp_THIGH_L, tmp_THIGH_R);
		//tmp_THIGHS = tmp_THIGH_R;
